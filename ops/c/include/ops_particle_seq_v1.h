...
template<typename ParamT> struct particle_param_handler {
  static char* construct(const ops_arg &arg, int dim, int ndim, ops_block block,
    ...
    return nullptr;
      ...
      int *binhead = (int *)map->parts_to_grid->data;
      block->instance->arg_idp[0] = binhead[0];
    }
...
//  size_t Nlist{0}; //, Nmax{OPS_MAX_PART};
  ...
  size_t nParticles = particle->no_particles + particle->no_virtual; //TODO: Vrf what is stored herein
  size_t no_particles = particle->no_particles;

  ops_arg args[N] = {arguments...};
  for (int i = 0; i < N; i++)
    if (args[i].argtype == OPS_ARG_DAT)
      throw OPSException(OPS_RUNTIME_ERROR,"Error: ops_particle_par_loop does not support particle/grid "
                                           "interactions. For such types of interaction, please use "
  ...
  case OPS_PARTICLE_ITERATE_RANDOM:
    n_loop_particles = getting_looping_particles_v2(nParticles, particle,
                                                    box, map_part,
                                                    dim, range_particles,
                                                    looping_particles);
    break;
...
//TODO: Remove some structures prior to loop keep only for elements-we can use only the particle.
  (void) std::initializer_list<int>{(initoffs_particles(arguments, offs[J], particle->block->dims,
