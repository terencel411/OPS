...
inline int get_mapping_address(int  *point,int dim, ops_dat binhead) {
  ...
  int prod = 1;
  for (int i = 0; i < dim; i++) {
    int ix = point[i] - binhead->d_m[i];
    if (ix < 0 || ix > binhead->size[i] - 1)
      return -1;
    address += ix * prod;
    prod *= binhead->size[i];
  }
...
inline int get_first_point(const int start[], const int dim,const ops_particle_mapping map,
  ...
  return (bin_head[address] > 0) ? bin_head[address] : 0;
  ...
  static char *construct(const ops_arg &arg, int dim, int ndim, int start[],
    ...
    if (arg.argtype == OPS_ARG_DAT_PARTICLE) {
      ...
      ACCP<T> *datap = new ACCP<T>(arg.dim, particle->no_particles + particle->no_virtual,
                                   (T *) arg.dat->data + ifirst); //TODO: Check
...
//  int igrid_to_map[OPS_MAX_DIM];
 ...
 //       printf("Point = [%d %d]:  to map [%d %d]\n", point[0], point[1], point2map[0], point2map[1]);

 //       printf("offs[0] = %d offs[1] = %d offs[2] = %d\n", offs[0], offs[1], offs[2]);
 //       printf("Point2Map [%d %d]\n", point2map[0], point2map[1]);
