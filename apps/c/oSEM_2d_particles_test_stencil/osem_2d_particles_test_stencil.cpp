...
static void report_ownership(ops_particle particle, ops_dat pos, ops_dat gid,
  ...
  if (nranks > 1)
    printf("  %d of the %d binned entries are ghosts, the same eddy held by more "
           ...
           tbin - eddies, tbin);
  fflush(nullptr);   // not stdout/NULL macros: they can crash the ops-translator parser
}
...
int main(int argc, char **argv) {
  ...
  // first build of the bins and the ghost band; the time loop updates them every step
  ...
  ops_printf("oSEM_2d_particles_test_stencil: %d eddies (decomposed by (y,z), "
             "reset into the position dat), grid %d x %d, %d steps\n",
  ...
  // time counters
  double c0, w0, c1, w1;
  double t_convect = 0, t_maps = 0, t_fluct = 0, t_rng = 0,  t_map_update = 0;
  ...
  for (int it = 1; it <= niter; it++) {
    ...
    // TEST: map update every step, so the reset moves the eddy and refreshes the ghosts
    {
    ...
    // grid-outer loop (ops_grid_part_seq_v2.h) through grid_part_loop.h, so the
    // translator leaves it alone. ops_par_particle_grid_loop only reads grid dats
    ops_par_grid_part_loop(compute_fluct, "compute_fluct", eddy_particle, map, S2D_SEARCH,
                 2, grid_range,
