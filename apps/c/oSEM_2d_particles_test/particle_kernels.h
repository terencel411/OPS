...
void KerConvectEddies(ACCP<double> &eddy_pos, ACCP<double> &eddy_yz, ACCP<double> &eddy_x, ACCP<double> &eddy_r,
  ...
  if (eddy_x(0) > x_max) {
    ...
    eddy_pos(0) = eddy_yz(0);   // TEST: reset into the position dat
    eddy_pos(1) = eddy_yz(1);
    eddy_eps(0) = (eddy_rng(3) < 0.5) ? -1.0 : 1.0;
    eddy_eps(1) = (eddy_rng(4) < 0.5) ? -1.0 : 1.0;
    eddy_eps(2) = (eddy_rng(5) < 0.5) ? -1.0 : 1.0;
    eddy_r(0) = eddy_radius;
