...
inline double shape_func_interpol(int ix, int iy, double *x_part, double *x_o,
                                  double dx) {

  double xcm = x_o[0] + 0.5 * dx;
  double ycm = x_o[1] + 0.5 * dx;

  double xi = (x_part[0] - xcm) / (0.5 * dx);
  double nu = (x_part[1] - ycm) / (0.5 * dx);
