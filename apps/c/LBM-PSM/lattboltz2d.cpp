...
void  implement_bc(ops_block block, ops_dat f, ops_dat xf,
                   ops_stencil S2D_00, double H, double uo, int size[]) {
 ...
 ops_par_loop(Ker_Diffuse_Refl_Norm_Pois_xpos, "Ker_Diffuse_Refl_Norm_Pois_xpos",
              block, 2, range_right,
              ops_arg_dat(f, 9, S2D_00, "double", OPS_WRITE),
              ops_arg_dat(xf, 9, S2D_00, "double", OPS_READ),
              ops_arg_gbl(uw, 2, "double", OPS_READ),
              ops_arg_gbl(&H, 1, "double", OPS_READ));


 //Left wall
  int range_left[] = {0, 1,  1, size[1] - 1};
  ops_par_loop(Ker_Diffuse_Refl_Norm_Poix_xneg, "KerDiffuse_Refl_Norm_yneg", block, 2,
               range_left,
               ops_arg_dat(f, 9, S2D_00, "double", OPS_WRITE),
               ops_arg_dat(xf, 9, S2D_00, "double", OPS_READ),
               ops_arg_gbl(uw, 2, "double", OPS_READ),
...
int main(int argc, char ** argv) {
    ...
    const int NX = 40*Ndx+1;
    const int NY = int (2.5*Ndx + 1.);
    int size[] = {NX, NY};
    ...
    double dx = 40.0/(size[0] - 1.0);
    double dx_f[2] = {dx, dx};

    double dx_c[2] = {40./(Nx_c - 1), 2.5/(Ny_c - 1)};
    ...
    printf("Dim 0: Stride (Dx_c/Dx) = %f (%d)\n", dx_c[0]/dx, (int) (dx_c[0]/dx));
    printf("Dim 1: Stride (Dx_c/Dx) = %f (%d)\n", dx_c[1]/dx, (int) (dx_c[1]/dx));


    int stride_prol[] = {Ndx, NY-1};
    int s2d_3pt[] = {-1, 0, 0, 0, 1, 0};
    ops_stencil S2D_9pt_prolong = ops_decl_prolong_stencil(2, 3, s2d_3pt, stride_prol, "PROLONG_00_10_10");


    //TODO:

    int NpH = (int) ceil(Ndx/2.0);
    int npoints = 2 * NpH + 1;


    printf("Number of points for restrict: %d\n", npoints);

    int size_sten = npoints * npoints;
    int *s2d_stride = (int *)ops_malloc(sizeof(int) * 2 * size_sten);
    for (int iy = 0; iy < npoints; iy++) {
      int jpoint = iy - NpH;
      for (int ix = 0; ix < npoints; ix++) {
        int ipoint = ix - NpH;
        int iloc = ix + iy * npoints;
        s2d_stride[2 * iloc] = ipoint;
        s2d_stride[2 * iloc + 1] = jpoint;
      }
    }

    ops_stencil S2D_9pt_restrict = ops_decl_restrict_stencil(2, size_sten, s2d_stride, stride_prol, "RESTRICT_00_10_10");
    ...
    double dx_box[] = {0., 0.};
    BoundingBox *bounding_box = ops_create_bounding_box(lb_block, xf_coarse, 2, dx_box);
    ...
    printf("xGrid: size =[%d %d]\n", x_grid->size[0], x_grid->size[1]);
   ...
   //=======================================
    ...
    ops_par_loop(KerInitMacros, "KerInitMacros", lb_block, 2,
                 full_range,
    ...
    ops_par_loop(KerInitMacrosPois, "KerInitMacrosPois", lb_block, 2, interior_range,
                 ops_arg_dat(rho, 1, S2D_00, "double", OPS_WRITE),
                 ops_arg_dat(u_x, 1, S2D_00, "double", OPS_WRITE),
                 ops_arg_dat(u_y, 1, S2D_00, "double", OPS_WRITE),
                 ops_arg_dat(x_grid, 2, S2D_00, "double", OPS_READ),
                 ops_arg_gbl(&u0, 1, "double", OPS_READ),
                 ops_arg_gbl(&H, 1, "double", OPS_READ));

    ops_par_loop(KerInitF,"KerInitF", lb_block, 2, interior_range,
                 ops_arg_dat(f, 9, S2D_00, "double", OPS_WRITE),
    ...
    double box_insert[] = {0.0, 20., 0., 2.5};
...
/*      ops_par_loop(KerCollision, "KerCollsion", lb_block,
                   2, interior_range,
                   ...
                   ops_arg_gbl(cx, 9, "double", OPS_READ),
                   ops_arg_gbl(cy, 9, "double", OPS_READ),
                   ops_arg_gbl(wi, 9, "double", OPS_READ),
                   ops_arg_gbl(&cs, 1, "double", OPS_READ)); */
      ...
      //BCs
      implement_bc(lb_block, f, x_grid, S2D_00, 2.5, u0, size);
