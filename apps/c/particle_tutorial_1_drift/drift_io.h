 ...
 * drift_io.h
 *
 *  Periodic HDF5 output for OPS Particles tutorial 1 (constant-velocity drift).
 *  This follows the pattern used by apps/c/oSEM/OPS_oSEM_io.h and
 *  apps/c/LBM-Particle_tracking/LBM_particle_io.h: every call produces one
 *  self-contained .h5 file holding the block, the grid ops_dats and the
...
inline void write_constants(const char *file_name,
  ...
  ops_write_const_hdf5("LENGTH", 1, "double", (char *)&params.LENGTH,
                       file_name);
  ...
  ops_write_const_hdf5("vel", 2, "double", (char *)params.vel, file_name);
  ops_write_const_hdf5("seed_box", 4, "double", (char *)params.seed_box,
                       file_name);
}

/**
 * Write one HDF5 file per output step: drift_output_<timestep>.h5
 */
inline void HDF5_IO_Write_drift_block_dynamic(
  ...
  char name0[80];
  sprintf(name0, "drift_output_%06d.h5", i + 1);
...
/**
 * Write the final state to a single file: drift_output.h5
 */
