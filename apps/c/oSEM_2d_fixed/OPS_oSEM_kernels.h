...
/* The global eddy id, straight from ops_arg_idx. This is what the particle
   app gets for free from its p_gid dat; a grid dat has no such thing, so one
   tiny kernel supplies it once and the random fill reads it exactly as
   ops_particle_random.h does. */
void instantiate_gid(ACC<int>& gid, const int* idx){
    gid(0, 0) = idx[0];
}
...
/* The rng dats are DOUBLE here, not int, and that one change fixes two
   defects at once while keeping ops_fill_random_uniform as the source.

   ops_lib_core.cpp:2581 dispatches on the dat's type:
       double -> std::uniform_real_distribution<double>(0.0, 1.0)
       int    -> std::uniform_int_distribution<int>(0, INT_MAX)

   On an INT dat the draw is never negative, so the reference's

       eps_x(0,0) = ((eps_x_rng(0,0) < 0) ? -1 : 1);

   returned +1 for every eddy, every step. SEM works by cancellation between
   random +/-1 signs; with every sign positive there is none, the fluctuations
   are not zero-mean, and the Reynolds stresses mean nothing.

   The same int draw was mapped to a position with

       (x_rng + 2147483648.0) / 4294967295.0

   which for the range [0, 2^31-1] spans [0.5000000001, 1.0] rather than
   [0, 1]. Every coordinate sat in the upper half of its range -- one eighth of
   the volume in 3-D -- and because convect_eddies re-seeded with the same
   expression it never washed out.

   With a double dat the draw is already uniform on [0,1), so the position is a
   direct affine map and the sign is a fair coin at 0.5. Same fill function,
   same call sites, no LCG of our own. */

void convect_eddies(ACC<double>& x, ACC<double>& y, ACC<double>& z, ACC<double>& radius, const ACC<double>& increment,
                    ACC<int>& eps_x, ACC<int>& eps_y, ACC<int>& eps_z, const ACC<double>& y_rng, const ACC<double>& z_rng,
                    const ACC<double>& eps_x_rng, const ACC<double>& eps_y_rng, const ACC<double>& eps_z_rng){
    x(0, 0) += increment(0, 0);
    if(x(0, 0) > x_max){
        x(0, 0) = x_min;
        y(0, 0) = eddy_y_min + (eddy_y_max - eddy_y_min) * y_rng(0, 0);
        z(0, 0) = eddy_z_min + (eddy_z_max - eddy_z_min) * z_rng(0, 0);
        eps_x(0, 0) = ((eps_x_rng(0, 0) < 0.5) ? -1 : 1);
        eps_y(0, 0) = ((eps_y_rng(0, 0) < 0.5) ? -1 : 1);
        eps_z(0, 0) = ((eps_z_rng(0, 0) < 0.5) ? -1 : 1);
...
void instantiate_eddies(ACC<double>& x, ACC<double>& y, ACC<double>& z, ACC<double>& radius, ACC<double>& increment,
    ACC<int>& eps_x, ACC<int>& eps_y, ACC<int>& eps_z, const ACC<double>& x_rng, const ACC<double>& y_rng, const ACC<double>& z_rng,
    const ACC<double>& eps_x_rng, const ACC<double>& eps_y_rng, const ACC<double>& eps_z_rng){
    x(0, 0) = x_min + x_rng(0, 0) * (x_max - x_min);
    y(0, 0) = eddy_y_min + y_rng(0, 0) * (eddy_y_max - eddy_y_min);
    z(0, 0) = eddy_z_min + z_rng(0, 0) * (eddy_z_max - eddy_z_min);
    radius(0, 0) = 0.2*delta;
    increment(0, 0) = u0 * dt;
    eps_x(0, 0) = ((eps_x_rng(0, 0) < 0.5) ? -1 : 1);
    eps_y(0, 0) = ((eps_y_rng(0, 0) < 0.5) ? -1 : 1);
