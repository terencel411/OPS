...
/* NOTE: these six were written as (char*)&x_gbl -- the address of the POINTER,
   not of the array. ops_write_const_hdf5 then read `eddies` elements starting
   at the pointer variable, so every eddy dataset in every frame was the
   pointer value followed by ~13 kB of adjacent memory. The `&` is dropped
   below. Nothing else in this function is changed. */
